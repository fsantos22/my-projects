...
const removeTeacher = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const findClass = await findData("teacher_class", "teacher_id", id);
    if (findClass) {
      const classId = findClass.class_id;

      // Remove o aluno da turma
      await deleteClassAssignment("teacher", id, classId);
    }
    ...
    res.send({ message: "Teacher removed from database!" });
  } catch (error) {
